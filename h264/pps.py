class PPS:
    ...
    def pic_parameter_set_rbsp(self):
        ...
        self.entropy_coding_mode_flag = self._bits.u(1)
        self.bottom_field_pic_order_in_frame_present_flag = self._bits.u(1)
        self.num_slice_groups_minus1 = self._bits.ue()
        if self.num_slice_groups_minus1 > 0 :
            self.slice_group_map_type = self._bits.ue()
            if self.slice_group_map_type == 0 :
                self.run_length_minus1 = []
                for i in range(self.num_slice_groups_minus1+1):
                    self.run_length_minus1.append(self._bits.ue())
            elif self.slice_group_map_type == 2 :
                self.top_left = []
                self.bottom_right = []
                for i in range(self.num_slice_groups_minus1):
                    self.top_left.append(self._bits.ue())
                    self.bottom_right.append(self._bits.ue())
            elif self.slice_group_map_type == 3 or \
                 self.slice_group_map_type == 4 or \
                 self.slice_group_map_type == 5 :
                self.slice_group_change_direction_flag = self._bits.u(1)
                self.slice_group_change_rate_minus1 = self._bits.ue()
            elif self.slice_group_map_type == 6 :
                self.pic_size_in_map_units_minus1 = self._bits.ue()
                self.slice_group_id = []
        ...
        self.num_ref_idx_l0_default_active_minus1 = self._bits.ue()
        self.num_ref_idx_l1_default_active_minus1 = self._bits.ue()
        self.weighted_pred_flag = self._bits.u(1)
        self.weighted_bipred_idc = self._bits.u(2)
        self.pic_init_qp_minus26 = self._bits.se()
        self.pic_init_qs_minus26 = self._bits.se()
        self.chroma_qp_index_offset = self._bits.se()
        self.deblocking_filter_control_present_flag = self._bits.u(1)
        self.constrained_intra_pred_flag = self._bits.u(1)
        self.redundant_pic_cnt_present_flag = self._bits.u(1)

        if self._bits.more_rbsp_data() :
            self.transform_8x8_mode_flag = self._bits.u(1)
            self.pic_scaling_matrix_present_flag = self._bits.u(1)
            ...
            self.second_chroma_qp_index_offset = self._bits.se()
